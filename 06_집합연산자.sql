...
from emp where dept_id in (10, 20, 30, 40)
union all
...
union all
select * from emp where dept_id in (20, 30);  --중복 허용
...
select 'MAX', max(salary) "급여집계" from emp
union all
...
select decode(grouping_id(job_id), 0, job_id, '총계') job_id, sum(salary)
from emp
