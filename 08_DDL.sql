...
*********************************************************************************** */
--no number primary key : default 이름 생성
create table parent_tb(
    no number constraint pk_parent_tb primary key,
    name nvarchar2(30) not null, 
    birthday date default sysdate,  --기본값 설정 : insert 시 값을 넣지 않았을 때 insert될 기본값  
    email varchar2(100) constraint uk_parent_tb_email unique, --unique 제약조건: 중복값X
    gender char(1) not null constraint ck_parent_tb_gender check(gender in ('M', 'F')) --check key: 값에 대한 제약
);
...
insert into parent_tb values(3, '홍길동2', null, 'b@b.com', 'M'); --명시적으로 null을 넣으면 default값이 아닌 null이 됨
insert into parent_tb values(4, '홍길동2', null, 'b@b.com', 'M');
insert into parent_tb values(5, '김영수', null, 'c@c.com', 'm');
...
************************************************************************************ */
--customers 카피해서 cust 만들기
--select 결과set을 테이블로 생성, (not null을 제외한 다른 제약조건은 카피가 안됨)
create table cust
as 
...
desc cust3;
--컬럼명 변경
alter table cust3 rename column cust_email to email; --cust_email => email로 변경
desc cust3;
--컬럼 삭제
...
alter table cust add constraint uk_cust_email unique(cust_email); --uk 추가
...
-- 1부터 1씩 자동증가하는 시퀀스
create sequence dept_id_seq; --이름 관례: 시퀀스사용할컬럼명_seq
...
-- -1부터 -1씩 자동 감소하는 시퀀스
create sequence ex4_seq increment by -1; --자동감소: start with 기본값이 -1
...
insert into emp2 values(emp_id_seq.nextval, '이순신', null, null, '2021/01/05', 30000, null, null, null);
insert into emp2 values(emp_id_seq.nextval, '박문수', null, null, '2021/01/05', 30000, null, null, null);
desc emp2;
