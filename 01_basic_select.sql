...
select [distinct] 컬럼명1, 컬럼명2 => 조회할 컬럼들을 지정. 테이블의 모든 컬럼을 조회하는 경우 *를 사용
                    => 컬럼명 별칭 : 별칭은 조회 결과로 보여질 컬럼의 이름. 별칭은 ""로 감쌀 수 있음(공백이 들어갈 경우 반드시 "" 사용)
                    => select 절이 distinct로 시작: 중복된 조회 결과는 하나만 보여줌
...
select emp_id 직원ID, --emp_id에서 조회, 결과는 직원 ID로 보여준다
        emp_name "직원 이름",
        hire_date 입사일,
        salary 급여,
        dept_name "소속 부서"
...
- 산술연산: +, -, *, /
- 연결연산자: 문자열을 붙이는 연산자. || => 모든 타입을 다 붙일 수 있다
...
from dual;  --dual: dummy 테이블 - select의 from절을 만들기 위해 사용하는 가상의 테이블

select 10+20, 20-5, 5*3, 10/2, mod(10,3) "10%3"
from dual;

select 'Sam'||'씨', 30000||'원'"가격"
from dual;

select 2000+100, null+100, null||'값'
from dual;

--sysdate: 실행 시점의 일시를 반환
select sysdate, sysdate+3"3일 후", sysdate+20"20일 후", sysdate-5"5일 전"
from dual;
...
select emp_name"직원 이름", '$'||salary"급여"
from emp;
...
select emp_id, emp_name, dept_name, job
from emp
where job = 'IT_PROG';
...
--extract (year|month|day from date)=2004; : date에서 year, month, day만 추출
...
select emp_id, emp_name, job, dept_name, comm_pct
from emp
...
select emp_id, emp_name, job, salary
from emp
order by 3, 4 desc;  --select 절의 3번째(job), 4번째(salary) 컬럼 기준 정렬(테이블 정렬순서 아님 select 순서)
